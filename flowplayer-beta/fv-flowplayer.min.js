...
flowplayer( function(api,root) {
  ...
  root.one( 'click', function() {
    if(
      api.conf.clip.sources[0].type.match(/dash/) ||
      api.conf.clip.sources[0].src.match(/vimeo\.com/) ||
      api.conf.playlist[0] && (
        api.conf.playlist[0].sources[0].type.match(/dash/) ||
        api.conf.playlist[0].sources[0].src.match(/vimeo\.com/)
      )
    ) { // todo: improve to check all sources and also Vimeo in FV Player Pro should be detected in a better way
      ...
      setTimeout( function() {  //  remove error message, show loading status and block further load attempts
        api.error = false;
        api.loading = true;
      ...
      jQuery.getScript(flowplayer.conf.script_dash_js, function() {
        api.loading = false;
        root.find('.fp-ui').click();  //  should not fire in case of FV PLayer Pro Vimeo, but what if Ajax finises before DASH.js loads? Then it would just play MP4. You need to prevent the FV Player Pro Ajax somehow!
      });
    }
  });
});
