...
*/
var FV_Player_Engine_Loader = { // we trigger and event on the player if there is check for it
  engineName: "fv-player-engine-loader",
  canPlay: function (type, conf) {    
    if( type == 'application/dash+xml' ) jQuery(conf.fv_player_engine_loader_root).trigger('fv-player-engine-load'); // todo: will Android choose MPD over MP4?
    return false;
  }
}
...
flowplayer( function(api,root) {
  ...
  api.conf.fv_player_engine_loader_root = root; // first we need to store a reference of the player
    
  root = jQuery(root);
  
  jQuery(root).on( 'fv-player-engine-load', function() {console.log('fv-player-engine-load handler');
