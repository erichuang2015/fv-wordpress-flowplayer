...
/*
Creates the missing event to allow us to laod the engine just on time
*/
flowplayer( function(api,root) { // first we need to store a reference of the player
  api.conf.fv_player_engine_loader_root = root;
});

var FV_Player_Engine_Loader = { // then we can trigger an even on that player once Flowplayer asks for engines
  engineName: "fv-player-engine-loader",
  canPlay: function (type, conf) {
    jQuery(conf.fv_player_engine_loader_root).trigger('fv-player-engine-load');
    return false;
  }
}
flowplayer.engines.unshift(FV_Player_Engine_Loader); // put it in as the first one

/*
Only load MPEG-DASH engine when it's going to be used. This will be a bigger problem on mobile one day
*/
flowplayer( function(api,root) {
  if( !flowplayer.conf.script_dash_js ) return;
  
  root = jQuery(root);
  
  jQuery(root).on( 'fv-player-engine-load', function() {
    var is_dash = api.conf.clip.sources[0].type.match(/dash/) || api.conf.playlist[0] && api.conf.playlist[0].sources[0].type.match(/dash/);
    var is_vimeo_dash = api.conf.clip.sources[0].src.match(/vimeo\.com/) || api.conf.playlist[0] && api.conf.playlist[0].sources[0].src.match(/vimeo\.com/);
    
    if( is_dash || is_vimeo_dash ) { // todo: improve to check all sources and also Vimeo in FV Player Pro should be detected in a better way
      var got_dash_already = false;
      jQuery(flowplayer.engines).each( function(k,v) {
        if( v.engineName == 'dash') got_dash_already = true;
      });      
      
      console.log('got_dash_already?',got_dash_already);
      
      if( got_dash_already ) return;      
      
      api.one( 'error', function() {  //  remove error message, show loading status and block further load attempts
        api.error = false;
        api.loading = true;
        root.removeClass('is-error').addClass('is-loading');        
        root.find('.fp-message').remove();      
      } );
      
      jQuery.getScript(flowplayer.conf.script_dash_js, function() { // loading the engine just in time will result in error unfortunately, here is a hack
        api.error = false;        
        root.removeClass('is-error').addClass('is-loading');        
        root.find('.fp-message').remove();      
        api.loading = false;
        if( !is_vimeo_dash ) root.find('.fp-ui').click();  //  prevrnt double Ajax for FV PLayer Pro Vimeo, but when Ajax finises before DASH.js loads is just plays MP4. You need to let FV Player Pro wait for DASH.js to become available
      });
    }
  });
});
